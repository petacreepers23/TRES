...
;    mov byte [0x0B8009], 1Bh
    ;mov byte [0x0B800A], 'P'
    ;mov byte [0x0B800B], 1Bh
    ;mov byte [0x0B800C], 'P'
    ;mov byte [0x0B800D], 1Bh
	;prueba hola m
    ;mov esi, 0xb8000 ; si = 0; si es el source index. sirve en general para apun a cadenas.
    ;mov [esi], byte 'H' ; metemos A en la dir de mem de si.
    ;add esi, 2 ; aumentamos el puntero por 2. 2 bytes.
    ;mov [esi], byte 'O'
    ;add esi, 2
    ;mov [esi], byte 'L'
    ;add esi, 2
...
CODE_SEG equ gdt_code - GDT_START ; x - (x+8) = 8
...
;times 400 - ($ - $$) db 0                       ; Rellenamos con 0.



[bits 32]
BOOT2_MP equ boot2 + 7c00h
boot2:
		;mov ax, 08h
        ;mov ds, ax
        ;mov ss, ax
